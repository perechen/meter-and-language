...
df <- read_tsv("data/temp/cs_lines.tsv") %>% 
...
counts <- df %>%
  ...
  count(label,pattern, pos, sort=T) %>% 
  group_by(label) %>% mutate(p=n/sum(n)) %>%  top_n(100) 
...
df2 %>% count(label,pattern, pos, per, sort=T) %>% group_by(label) %>% mutate(p=n/sum(n))
